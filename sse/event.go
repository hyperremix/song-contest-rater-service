...
func NewEvent(publicEvent EventOptions) (Event, error) {
	privateEvent := Event{}

	if publicEvent.Data != nil {
		json, err := json.Marshal(publicEvent.Data)
		if err != nil {
			return Event{}, err
		}
		privateEvent.Data = json
	}

	if publicEvent.ID != "" {
		privateEvent.ID = []byte(publicEvent.ID)
	}

	if publicEvent.Event != "" {
		privateEvent.Event = []byte(publicEvent.Event)
	}

	if publicEvent.Retry != 0 {
		privateEvent.Retry = []byte(strconv.Itoa(publicEvent.Retry))
	}

	if publicEvent.Comment != "" {
		privateEvent.Comment = []byte(publicEvent.Comment)
	}

	return privateEvent, nil
}
